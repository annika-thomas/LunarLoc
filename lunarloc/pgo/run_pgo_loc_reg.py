...
from utils.datasets import extract_orbslam, extract_gt, tf_at_frame, extract_imu
...
def load_registrations(json_path):
    with open(json_path, "r") as f:
        return json.load(f)

def add_registration_factors(graph, initial, registrations,
                             orbslam_frames, orbslam_poses,
                             dummy_offset=10_000,
                             LOCALIZATION_NOISE=None):
    """
    Replace dummy GT priors with registration-based localization factors.
    """
    for r in registrations:
        frame_id = int(r["frame"])
        R_est = np.array(r["R_est"])
        t_est = np.array(r["t_est"]).reshape(3)
        ...
        # gtsam Pose3
        T_w_est = gtsam.Pose3(T_est)

        # Find the trajectory index for this frame_id
        idxs = np.where(orbslam_frames == frame_id)[0]
        if len(idxs) == 0:
            continue
        i = int(idxs[0])

        # Create an auxiliary "localization node" at the estimated pose
        Y_i = Y(dummy_offset + i)
        initial.insert(Y_i, T_w_est)

        # Between factor: enforce X(i) ~ Y_i up to noise
        graph.add(gtsam.BetweenFactorPose3(
            X(i),
            Y_i,
            gtsam.Pose3(),  # identity transform, since we want them equal
            LOCALIZATION_NOISE
        ))
...
def main(first_traverse: FrameDataReader, silent: bool = False):
    ...
    GT_ANCHOR_NOISE = gtsam.noiseModel.Diagonal.Sigmas(
        np.array([1e-3, 1e-3, 1e-3,   # rot
                1e-3, 1e-3, 1e-3],  # trans
                dtype=np.float64)
    )
    ...
    # localization_idx = set(range(0, N, 100))
    ...
    # Precompute mapping from frame_id → index for efficiency
    frame_to_idx = {int(fid): idx for idx, fid in enumerate(orbslam_frames1)}

    used_frames = set()


    def as_pose3(T: np.ndarray) -> gtsam.Pose3:
        return gtsam.Pose3(gtsam.Rot3(T[:3, :3]), T[:3, 3])

    GT_ANCHOR_NOISE = gtsam.noiseModel.Diagonal.Sigmas(
        np.r_[np.deg2rad([0.5, 0.5, 0.5]), [0.005, 0.005, 0.005]]
    )  # [rot (rad), trans (m)]


    # Instead of building Y_i + prior + between, just do this:
    ...
    for r in registrations:
        ...
        if frame_id not in frame_to_idx:
            print(f"Frame {frame_id} not in orbslam_frames1")
            frame_id += 5
        i = frame_to_idx[frame_id]

        # ORB-SLAM estimated pose as 4x4
        T_w_orb = orbslam_estimates1[i]   # (4,4) numpy array

        # Registration transform (from JSON)
        R_est = np.asarray(r["R_est"])
        t_est = np.asarray(r["t_est"]).reshape(3)
        T_reg = np.eye(4)
        T_reg[:3, :3] = R_est
        T_reg[:3, 3]  = t_est

        # ✅ Apply correction directly in numpy
        # If registration is ORB→GT, then GT = T_reg @ ORB
        T_w_corr = T_reg @ T_w_orb

        # Convert to gtsam.Pose3 only at the end
        T_w_corr = as_pose3(T_w_corr)

        # Ground truth pose for this frame
        T_w_gt = as_pose3(tf_at_frame(first_traverse[frame_id]))

        # Insert a prior directly on the trajectory node X(i)
        graph.add(gtsam.PriorFactorPose3(
            X(i),
            T_w_gt,
            GT_ANCHOR_NOISE   # strong but not infinite noise model
        ))

        print(f"anchored X({i}) to GT at frame_id={frame_id}")
    ...
    #     # (Optional: compare to GT pose)
    #     T_w_gt = as_pose3(tf_at_frame(first_traverse[frame_id]))

    #     # print("gt: ", T_w_gt)
    #     # print("corr: ", T_w_corr)

    #     err_t = np.linalg.norm(T_w_corr.translation() - T_w_gt.translation())
    #     print(f"[frame {frame_id} idx {i}] Δt={err_t:.3f} m")

    #     # Fix node at corrected pose
        
    #     Y_i = X(dummy_offset + i)
    #     initial.insert(Y_i, T_w_gt)
    #     graph.add(gtsam.PriorFactorPose3(Y_i, T_w_gt, GT_ANCHOR_NOISE))
    #     graph.add(gtsam.BetweenFactorPose3(X(i), Y_i, gtsam.Pose3(), LOCALIZATION_NOISE))

    #     print(f"added global localization at frame_id={frame_id}, index={i}")
