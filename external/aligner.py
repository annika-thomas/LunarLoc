...
gt_rock_locations = gt_rock_locations[(gt_rock_locations[:, 0] >= -7) & (gt_rock_locations[:, 0] <= 20) &
                                      (gt_rock_locations[:, 1] >= -12) & (gt_rock_locations[:, 1] <= 0)]
...
# ax.plot(poses4[:, 0], poses4[:, 1], poses4[:, 2], 'r-', alpha=0.8, label='Path 4')
# ax.plot(poses5[:, 0], poses5[:, 1], poses5[:, 2], 'c-', alpha=0.5, label='Path 5')
ax.plot(poses6[:, 0], poses6[:, 1], poses6[:, 2], 'b-', alpha=0.8, label='Path 6')
# ax.plot(poses7[:, 0], poses7[:, 1], poses7[:, 2], 'k-', alpha=0.5, label='Path 7')
ax.plot(poses8[:, 0], poses8[:, 1], poses8[:, 2], 'g-', alpha=0.8, label='Path 8')
...
ax.plot(poses13[:, 0], poses13[:, 1], poses13[:, 2], 'y-', alpha=0.8, label='Path 13')
...
print("boulders in 4:", boulders4.shape)
print("boulders in 6:", boulders6.shape)
print("boulders in 8:", boulders8.shape)
print("boulders in 13:", boulders13.shape)
...
for countouter in range(1, 16):
    for countinner in range(1, 16):
        ...
        traverse_1_data = mission_data[countouter-1]
        traverse_2_data = mission_data[countinner-1]

        boulder_locations1 = traverse_1_data[-1][1] 
        ...
        # headers = ['trav1', 'trav2', 'num_boulders1', 'num_boulders2', 'num_associations', 'translation error', 'rot error (deg)', 'rot error (rad)']
        row = [countouter, countinner, len(boulder_locations1), len(boulder_locations2), Ain.shape[0], translation_error, theta_error_deg, theta_error]
        rows.append(row)

with open('clipper_output.csv', 'w', newline='') as f:
    writer = csv.writer(f)
